# notes.pdf

## Table of contents

- [About](#about)
- [Categories](#categories)
  - [Artificial Intelligence](#artificial-intelligence)
  - [Infrastructure](#infrastructure)
  - [Robotics](#robotics)

## About 
> Collection of personal thoughts about the future, from A.I to robotics and beyond.
> They are rough and therefore, don't expect them to be as polished as a real article.

## Categories

| Category | Short description |
|----------|-------------------|
|Artificial intelligence|Everything related to A.I and its applications.|
|Infrastructure|Modern infrastructure for a better future.|
|Robotics|Robots to automate manual labor.|

### Artificial Intelligence

| Note     | Short description |
|----------|-------------------|
|[Online market for A.I models](https://github.com/boristheprogrammer/notes.pdf/blob/main/artificial_intelligence/online-market-for-ai-models.md)|Enhancing the capabilities of AI agents through custom models sold online.|

### Infrastructure

| Note     | Short description |
|----------|-------------------|
|[Autonomous Electric Trains](https://github.com/boristheprogrammer/notes.pdf/blob/main/infrastructure/autonomous-trains.md)|Providing a safe and reliable main mobility mechanism for humans.|

### Robots

| Note     | Short description |
|----------|-------------------|
|[Why we need more bots?](https://github.com/boristheprogrammer/notes.pdf/blob/main/robotics/more-robots.md)|A case for more robots.|
